...
<table id="tugas" class="table table-bordered table-striped">
    ...
    <tbody>
        ...
        ?>
        <?php foreach($tugas_share->result() as $result) : ?>
        <tr>
            ...
            <td class="text-center"><?php echo hariIndo(date("l", strtotime($result->tgl_selesai))).' '.date("d-m-Y H:i:s", strtotime($result->tgl_selesai)) ?></td>
...
<script>
    ...
    $(".lihat-data").click(function(e) {
      id = $(this).data('id');
      location.href = base_url+`tugasshare/detail/${id}`;
    });
    $(".hapus-data").click(function(e) {
      ...
      function(isConfirm) {
        if (isConfirm) {
          $.ajax({
             url: '<?=site_url('tugasshare/crud/delete')?>',
             type: 'POST',
             dataType: 'json',
             data: {
               id: id,
