...
class JadwalSiswaModel extends CI_Model {
	...
	{
		$this->db->select('m.mapel,h.hari,j.id_detail_hari,TIME_FORMAT(j.jam_mulai, "%H:%i") AS jam_mulai,TIME_FORMAT(j.jam_selesai, "%H:%i") AS jam_selesai,g.nama');
        ...
        $this->db->join('hari_masuk_detail h2', 'h2.id=j.id_detail_hari');
        $this->db->join('hari_masuk h','h.id=h2.id_hari');
        ...
        $this->db->order_by('j.id_detail_hari');
